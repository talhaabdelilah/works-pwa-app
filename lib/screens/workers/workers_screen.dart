...
import 'package:project_manager/screens/workers/worker_detail_screen.dart';
import 'package:project_manager/widgets/name_suggestions_field.dart';
...
class _WorkersScreenState extends State<WorkersScreen> {
  ...
  String _getWeekKey(DateTime d) {
    ...
    final startOfYear = DateTime(sat.year, 1, 1);
    final dayOfYear = sat.difference(startOfYear).inDays;
    final satDow = sat.weekday == 7 ? 6 : sat.weekday - 1;
    final offsetDays = (6 - satDow + 7) % 7;
    final weekNum = ((dayOfYear + offsetDays) / 7).ceil();
    return '${sat.year}-W${weekNum.toString().padLeft(2, '0')}';
  }
  ...
  String _formatWeek(String wk) {
    try {
      ...
      return '${firstSat.day}/${firstSat.month} - ${fri.day}/${fri.month} $year';
    } catch (_) {
      return wk;
    }
  }
  ...
  void _prevWeek() {
    final parts = _currentWeek.split('-W');
    ...
    final jan1Dow = jan1.weekday == 7 ? 6 : jan1.weekday - 1;
    final daysToSat = (6 - jan1Dow + 7) % 7;
    final sat = jan1.add(Duration(days: daysToSat + (week - 1) * 7 - 7));
    setState(() => _currentWeek = _getWeekKey(sat));
  }
  ...
  void _addWorker() {
    ...
    final existingNames = widget.userData.workersByWeek.values
        .expand((list) => list)
        .map((w) => w.name)
        .toList();
    showDialog(
            ...
            ElevatedButton(
              onPressed: () {
                ...
                _save([Worker(
                  ...
                  name: nc.text.trim(),
                  price: double.tryParse(wc.text.trim()) ?? 0,
                ), ..._getWorkers()]);
                Navigator.pop(ctx);
  ...
  void _toggleDay(Worker w, String dayKey) {
    w.days[dayKey] = (w.days[dayKey] == 1) ? 0 : 1;
    _save(_getWorkers());
  }
  ...
  Widget build(BuildContext context) {
    ...
    final todayDow = today.weekday == 7 ? 6 : today.weekday - 1;
    final todayKey = Worker.dayKeys[todayDow == 0 ? 6 : todayDow - 1];
    final presentToday = workers.where((w) => w.days[todayKey] == 1).length;
    final totalWages = workers.fold(0.0, (s, w) => s + w.weeklyTotal);
              ...
              child: filtered.isEmpty
                  ...
                  : LayoutBuilder(
                      builder: (context, constraints) {
                        ...
                        final idxW = 24.0;
                        final dayW = (totalW - idxW - 100) / 10;
                        final nameW = dayW * 1.8;
                        final totalW2 = dayW * 1.2;
                        return Container(
                            ...
                            child: SizedBox(
                              ...
                              child: DataTable(
                                ...
                                columns: [
                                  ...
                                  DataColumn(label: SizedBox(width: nameW, child: const Text('الاسم', style: TextStyle(fontSize: 9, fontWeight: FontWeight.bold, color: Colors.white)))),
                                  ...List.generate(7, (i) => DataColumn(
                                    label: SizedBox(width: dayW, child: Text(_dayShort[i], style: const TextStyle(fontSize: 8, fontWeight: FontWeight.bold, color: Colors.white), textAlign: TextAlign.center)),
                                  )),
                                  DataColumn(label: SizedBox(width: totalW2, child: const Text('الإجمالي', style: TextStyle(fontSize: 8, fontWeight: FontWeight.bold, color: Colors.white), textAlign: TextAlign.center))),
                                ...
                                rows: List.generate(filtered.length, (i) {
                                  ...
                                  return DataRow(cells: [
                                    ...
                                    ...List.generate(7, (j) {
                                      final dk = Worker.dayKeys[j];
                                      final present = w.days[dk] == 1;
                                      return DataCell(SizedBox(
                                        width: dayW,
                                        child: GestureDetector(
                                          onTap: () => _toggleDay(w, dk),
                                          child: Center(
                                            child: Container(
                                              width: 16, height: 16,
                                              decoration: BoxDecoration(
                                                shape: BoxShape.circle,
                                                color: present ? const Color(0xFF10B981) : const Color(0xFFE2E8F0),
                                              ),
                                              child: present ? const Icon(Icons.check, size: 10, color: Colors.white) : null,
                                            ),
                                          ),
                                        ),
                                      ));
                                    ...
                                    DataCell(SizedBox(
                                      ...
                                      child: Container(
                                        ...
                                        child: Text('${w.weeklyTotal.toStringAsFixed(0)}', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 8, color: w.weeklyTotal >= 0 ? const Color(0xFF065F46) : const Color(0xFF991B1B)), textAlign: TextAlign.center),
                                      ),
                                    )),
                                    DataCell(
                                      GestureDetector(
